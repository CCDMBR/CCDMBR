...
class AuxiliaryGuidedDiffusion:
    def __init__(self, mean_type, noise_schedule, noise_scale, noise_min, noise_max,
        ...
        super(AuxiliaryGuidedDiffusion, self).__init__()
    ...
    def auxiliary_guided_training_losses(self, model, target_user_embs, auxiliary_condition, reweight=False):
        batch_size, device = target_user_embs.size(0), target_user_embs.device
        ts, pt = self.sample_timesteps(batch_size, 'importance')
        noise = th.randn_like(target_user_embs)

        if self.noise_scale != 0.:
            x_t = self.q_sample(target_user_embs, ts, noise)
        else:
            x_t = target_user_embs

        terms = {}
        model_output = model(x_t, auxiliary_condition, ts)

        target = {
            ModelMeanType.START_X: target_user_embs,
            ModelMeanType.EPSILON: noise,
        }[self.mean_type]

        assert model_output.shape == target.shape == target_user_embs.shape
        ...
        if reweight == True:
            if self.mean_type == ModelMeanType.START_X:
                ...
                likelihood = mean_flat(
                    (target_user_embs - self._predict_xstart_from_eps(x_t, ts, model_output)) ** 2 / 2.0)
    ...
    def auxiliary_guided_p_sample(self, model, x_start, auxiliary_condition, steps, sampling_noise=False):
        ...
        if self.noise_scale == 0.:
            for i in indices:
                ...
                x_t = model(x_t, auxiliary_condition, t)
            return x_t

        for i in indices:
            t = th.tensor([i] * x_t.shape[0]).to(x_start.device)
            out = self.p_mean_variance(model, x_t, auxiliary_condition, t)
            if sampling_noise:
    ...
    def p_mean_variance(self, model, x, auxiliary_condition, t):
        B, C = x.shape[:2]
        assert t.shape == (B,)
        model_output = model(x, auxiliary_condition, t)
